...
当前项目版本号为 **0.3.0**，以 [docs/02-roadmap.md](docs/02-roadmap.md) 的 `v0.3 — 数据解析与缓存` 为基线。
...
- Windows CLI 基础入口：`version`、`login/logout/whoami`、`course`、`exam`、`classroom`、`term list`、`week list`。
- 单元测试与文档结构。

仓库中已经存在 v0.4+ 方向的代码和文档草案，但这些内容应视为后续阶段或实验性工作；除非对应路线图阶段完成并更新版本号，否则不代表当前稳定承诺。

## 后续阶段

路线图中的后续阶段包括：

- **v0.4 — CLI 工程化**：命令树稳定化、统一 JSON 输出、固定 exit code、配置/缓存命令、CLI 集成测试。
...
以下示例以 v0.3 基线能力为主，优先使用 mock/offline 路径：
