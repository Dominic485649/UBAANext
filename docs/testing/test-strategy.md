...
课堂资源 smoke 报告必须区分：缺少凭据、校园网/后端不可达、direct TLS 不可用但 VPN 可用、指定日期资源为空、资源缺少 PPT GUID、视频是 HLS 但本机没有 `ffmpeg`、下载后只生成 `.m3u8.url` sidecar、以及本地临时目录清理失败。默认 `ctest` 不访问真实课堂资源后端。
