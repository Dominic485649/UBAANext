...
| `tools/live-smoke.ps1` | 手动 live smoke runner；L1 为只读，Cloud 可逆写 smoke 需显式 `UBAANEXT_CLOUD_WRITE=1` 和写确认开关。 |
...
集成测试和 live smoke 的推进顺序应与 `docs/reports/buaa-api-migration-matrix.md` 保持一致：
...
5. 写操作默认仅验证“未确认时拒绝执行”“平台未声明 `write_operations` 时拒绝执行”和“需要显式确认”的本地契约；Cloud 可逆写 smoke 可在显式开关下创建临时目录/小文件/分享并尽力清理。
...
- L2/L3 即使启用，也不由 runner 自动执行高风险真实写操作；用户必须手动指定具体命令并确认风险。Cloud 可逆写 smoke 是唯一 runner 内置写路径，且必须额外设置 `UBAANEXT_CLOUD_WRITE=1`、`UBAANEXT_ALLOW_WRITE=1`、`UBAANEXT_CONFIRM_WRITE=1`。
...
- `file upload` 和 `ygdk submit --photo` 等 typed 上传写操作必须单独验证本地文件读取失败、缺少确认时 fail-closed；Cloud 上传可进入显式可逆写 smoke，其他业务上传不进入默认 live smoke。
