...
- 课堂资源只读接口：`ubaanext_live_week(context, start_date, end_date)`、`ubaanext_live_resources(context, date, status, from_course)`、`ubaanext_live_detail(context, course_id, sub_id, date)`。这些接口把 BBUAA 差异补齐后的课堂直播/回放/PPT 资源入口暴露给 Harmony/NAPI；`live_resources` 的 `from_course` 会复用 Core 课表查询做保守过滤，`live_detail` 会通过 CookieStore 派生 live token 并降级解析 livingroom HTML。
...
- 本地写性质实验接口：`ubaanext_td_image_delete(context, name, force, confirmed)`。该接口只删除 TD 本地图片，不访问真实 TD 服务器；必须传入 `confirmed=1`，默认拒绝删除仍被 TD 用户引用的图片，只有 `force=1` 才允许解除引用保护，但仍不得绕过安全文件名校验。
- 远端写性质实验接口：`ubaanext_signin_do(...)`。该接口必须继续通过 `confirmed` 入参、`PlatformCapabilities::write_operations` 和 Core `WriteOperationGate`；UI 侧不得把它视为可直接写能力。
...
- `ubaanext_live_week(...)` 与 `ubaanext_live_resources(...)` 在 mock/offline 下返回稳定 envelope；`ubaanext_live_detail(...)` 参数缺失时返回 `InvalidArgument`，不触发真实网络。
...
- TD 本地只读接口在空本地 store 下返回 `ok:true` 与空数组，而不是触发远端网络；`ubaanext_td_image_delete(...)` 未确认时失败，删除不存在图片时返回 `accepted:false` 的成功 mutation，方便 ArkUI 呈现“无可删文件”。
...
- 课堂资源下载落盘、PPTX 生成和 HLS/ffmpeg 合并；这些仍由 CLI 或更高层文件系统能力处理，C ABI 本轮只暴露只读发现与详情。
- 通用文件上传。
