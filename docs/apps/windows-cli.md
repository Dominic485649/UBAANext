...
> 当前版本阶段：`v0.3.0`。本页记录 CLI 的目标形态和已实现/规划命令；v0.3 当前基线以 mock/offline、课程/考试/教室/学期/教学周解析与缓存为稳定范围。真实登录、写操作和完整 CLI 工程化属于 v0.4+ / v0.5+ 后续阶段，使用前应以 `ubaa help` 和测试覆盖为准。
...
| :--- | :--- | :--- |
| `--username <id>` | `login` | 用户的学号 / 账户 ID。 |
| `--password <pw>` | `login` | 用户登录密码。该选项在控制台及日志中必须进行数据脱敏。 |
...
*   **`ubaa version`**
    *   **描述**：显示当前可执行程序的编译版本。
    *   **人类可读输出**：`UBAA Next 0.3.0`
    *   **JSON 格式**：`{"ok":true,"data":{"version":"0.3.0"},"error":null}`
*   **`ubaa help`**
    *   **描述**：打印标准命令行帮助手册。
*   **`ubaa login --username <id> --password <pw>`**
