...
2. **真实只读 UI**：需要先完成 NAPI/C API 边界合同，包括 typed service 映射、错误码映射、异步 Promise、redaction、capability 查询和 partial failure 展示合同。当前 native bridge 已准备好课程/成绩/考试/Todo/YGDK，以及课堂资源 `liveWeek/liveResources/liveDetail` 的只读入口，ArkTS 不应直接实现 classroom/yjapi/livingroom 协议。
...
- TD 本地图片删除可通过 NAPI `tdImageDelete` 进入 C ABI，但必须在 UI 层二次确认；默认不得把它和 TD 远端打卡、自动调度、后台刷新混为一类能力。
