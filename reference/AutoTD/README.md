...
```bash
pip install autotd-buaa==0.1.15
```
...
从 `0.1.7` 开始，AutoTD 默认开启 telemetry，用于向维护者的 Cloudflare Worker + D1 后台上报安装、运行、停止、用户变化、TD 次数变化和后台运行跨日快照。`0.1.15` 默认 endpoint 为 `https://autotd-telemetry.autotd-buaa.workers.dev`。
...
`autotd ui` 会启动本地 Web 管理端并打开浏览器。页面可查看用户列表、今日打卡进度、TD 数和后台 PID，支持 quick 新增/删除用户、运行或停止后台进程、执行 `autotd run --once`，以及上传、预览和删除本地图片。
